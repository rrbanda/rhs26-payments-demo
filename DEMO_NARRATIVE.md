...
>
> The pod also carries Red Hat Agent Operator labels that enable additional security layers when deployed to a fully configured cluster — AuthBridge sidecar for network-edge JWT enforcement, and SPIFFE/SPIRE for workload identity between services. Those are platform-level capabilities that layer on top of the application auth, same agent code, no changes needed.
>
