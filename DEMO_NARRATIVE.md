...
>
> At the infrastructure level, this cluster runs SPIFFE/SPIRE through OpenShift's Zero Trust Workload Identity Manager — SPIRE server, SPIRE agents on every worker node, SPIFFE CSI driver, and an OIDC discovery provider. That gives every workload a cryptographic identity. The agent pod carries Red Hat Agent Operator labels that integrate it with this identity mesh. Keycloak client registration uses client secrets authenticated through this infrastructure. So you have two security layers working together: application-level JWT auth that the agent code enforces, and infrastructure-level workload identity that the platform manages transparently.
>
